...
def test_example_config_resolves_paths() -> None:
    config = load_config(Path(__file__).parents[1] / "configs" / "kawaii_future_bass.toml")
    assert config.dataset.audio_directory.name == "YouTube"
...
def test_abc_config_enables_full_symbolic_training() -> None:
    config = load_config(Path(__file__).parents[1] / "configs" / "kawaii_future_bass_abc.toml")
    assert config.dataset.require_abc
    assert config.train.cot == "full"
    assert config.train.output_directory.name == "kawaii_future_bass_abc"


def test_invalid_generated_fraction(tmp_path: Path) -> None:
    text = (Path(__file__).parents[1] / "configs" / "kawaii_future_bass.toml").read_text()
